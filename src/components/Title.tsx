...
export const Title = ({
  ...
  return <div className={cn("transition-all duration-500", isRecording ? "relative" : "absolute w-full top-[calc(50%-180px)]")}>
      <div className="flex items-center justify-center gap-2">
        <h1 className={cn("font-bold tracking-tight bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent transition-all duration-500", isRecording ? "text-4xl" : "text-6xl")}>
          CoCo
        </h1>
        <Sparkles className={cn("text-blue-500 animate-pulse transition-all duration-500", isRecording ? "w-6 h-6" : "w-10 h-10")} />
      </div>
